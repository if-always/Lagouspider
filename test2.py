...
def get_url(p):  #传入爬取的城市号
    if p==0:
        url='https://www.lagou.com/jobs/list_python%E7%88%AC%E8%99%AB?px=default&city='+str(
            '%E4%B8%8A%E6%B5%B7#filterBox')  #深圳
    elif p==1:
        url = 'https://www.lagou.com/jobs/list_python%E7%88%AC%E8%99%AB?px=default&city=' + str(
            '%E5%B9%BF%E5%B7%9E#filterBox')  #广州
    else:
        url='https://www.lagou.com/jobs/list_python%E7%88%AC%E8%99%AB?px=default&city=' + str(
...
def get_content():
    ...
    for p in range(3):
        ...
        response = requests.get(get_url(p), headers=headers1)  #获取不同城市的源码
        if p==0:
            city_code='\u6df1\u5733'
        elif p==1:
            city_code='\u5e7f\u5dde'
        ...
        for i in range(1,30):#最多页数的深圳也只有3页！
            # ajax网页的请求头，和之前的不一样！
            headers = {
            ...
            params = {
                ...
                'city': city_code  # 城市转换编码后的结果
            }
            data = {
                'kd': '\u0050\u0079\u0074\u0068\u006f\u006e\u722c\u866b',  # Python爬虫转换编码后的结果，关键字
        ...
        #但是我们加上formdata中的data后，再次请求就可以了
            ...
            for i in result:
                ...
                try:
                    businessZones=','.join(i['businessZones'])  #公司区域,也是个列表
                except:
                    businessZones='None'
                print(positionName)
            time.sleep(2)
